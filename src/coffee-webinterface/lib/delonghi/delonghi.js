...
import { rxtxBuffers } from './buffers'
...
export class Delonghi {
  constructor (transport = (() => { throw new Error('transport is required') })(), ksy = (() => { throw new Error('schema is required') })()) {
    ...
    this.state = new DelonghiState(this)
  ...
  parseData (data) {
    ...
    //   this.setState(({ buffers = {}, ...rest }) => {
      Object.entries(packets)
        .forEach(([key, value]) => {
          const [type, use] = key.toLowerCase().split(':')
        ...
        //   .map((key) => key.split(':'))
        //   .filter(([type, buffer]) => rxtxBuffers[type] && typeof rxtxBuffers[type][buffer] !== 'undefined')
...
*/
  ...
  disableEntkalken = () => {
    ...
    filters.and = 'FF'.repeat(this.PACKET_LEN)
    filters.or = '00'.repeat(this.PACKET_LEN)
